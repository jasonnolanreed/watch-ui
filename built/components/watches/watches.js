...
export class Watches extends GWBWElement {
    ...
    onChangeSort(event, target) {
        let newSortValue = target.classList.contains(`name`) ? `name` :
            target.classList.contains(`measured`) ? `measured` :
                `created`;
        if (target.classList.contains(`name`) && this.preferences.watchesSort.includes(`name`)
            ||
    ...
    sortWatches() {
        const sortField = this.preferences.watchesSort.includes(`name`) ? `name` :
