...
export const parseSessionsFromMeasures = (allMeasures) => {
    ...
    for (const measure of allMeasures) {
        ...
        if (!measure.firstOfSession) {
            ...
            if (measuresOfSession.length) {
                sessions.push(measuresOfSession);
            }
...
export const getSessionTotalData = (session) => {
    ...
    const sessionDistance = Difference.days(session[0].targetMoment, session[session.length - 1].targetMoment);
    const sessionDrift = getMomentDiffFromMeasure(session[session.length - 1]) - getMomentDiffFromMeasure(session[0]);
    return {
