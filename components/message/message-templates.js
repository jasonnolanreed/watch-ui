...
const getTypeIcon = component => {
	let iconName = ``;
	if (component.getAttribute(`type`) === `info`) { iconName = `info`; }
	if (component.getAttribute(`type`) === `success`) { iconName = `check_circle`; }
	if (component.getAttribute(`type`) === `error`) { iconName = `warning`; }
